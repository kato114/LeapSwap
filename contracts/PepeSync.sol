...
contract PepeSyncReward is IREWARD {   
    ...
    IERC20 public USDC = IERC20(0xadf393f388BBd5EFbA07987dd734467F7c6e81f0);
    ...
    event Ownershiptransfer(address indexed _previous,address indexed _newOwner);

    constructor(address _divOwner) {
        router = IRouter(0x66b2FAFc8c5FC3A85d845804d156e9DEd21f321D);
        _token = msg.sender;
        _owner = _divOwner;
    }
    ...
    function deposit() external payable override onlyToken {
        ...
        uint iBalance = IERC20(USDC).balanceOf(address(this));
        ...
        uint rBalance = IERC20(USDC).balanceOf(address(this));

        uint256 amount = rBalance.sub(iBalance);
    ...
    function setUsdc(address _adr) external onlyOwner {
        USDC = IERC20(_adr);
    }
    ...
    function claim() external {
        ...
        if(subtotal > 0) {
            ...
            IERC20(USDC).transfer(user, subtotal);
        }
    }
...
contract PepeSync is ERC20Burnable, ERC20Votes, Ownable {

    using SafeMath for uint256;

    IRouter public _router = IRouter(0x66b2FAFc8c5FC3A85d845804d156e9DEd21f321D);
    ...
    PepeSyncReward public distributor;

    bool public swapEnabled = false;
    bool public swapLimit;
    bool private swapping;

    constructor() ERC20("PepeSync", "SYNC") ERC20Permit("PepeSync") {
        distributor = new PepeSyncReward(msg.sender);
        ...
        _isExcludedFromFees[zeroAddress] = true;
    ...
    function _transfer(
        ...
        if (takeFee) {
            // on sell
            if (isMarketPair[to] && sellFee > 0) {
                fees = amount.mul(sellFee).div(denominator);
            }
    ...
    function setFee(uint _newSellFee) public onlyOwner {     
        sellFee = _newSellFee;
    }

    function setExcludeFromFee(address _adr,bool _status) public onlyOwner {
        require(_isExcludedFromFees[_adr] != _status,"Not Changed!!");
        _isExcludedFromFees[_adr] = _status;
    }
    ...
    function changeRouter(address _newRouter) external onlyOwner {
        _router = IRouter(_newRouter); 
    }

    function swapBack(uint contractBalance) private {
        ...
        if(rbalance > 0) try distributor.deposit{value: rbalance}() {} catch {}
    }
    ...
    // do not add pair and token address in reward
